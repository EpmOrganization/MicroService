       ...
       .FallbackAsync(
            ...
            Task.Run(async () =>
            {
                for (int i = 0; i < 50; i++)
                {
                    ...
                    // await PolicyBuilder.CreatePolly().ExecuteAsync(() => new Test().HttpInvokeAsync());
        ...
        static async  Task<string>  Get(IConsulHttpClient _consulHttpClient, UserServiceConfig userServiceConfig)
        {
            return JsonConvert.SerializeObject(await _consulHttpClient.GetAsync<ApiResponseWithData<List<User>>>(userServiceConfig.ServiceName, userServiceConfig.GetUri));
        }
    }
}
